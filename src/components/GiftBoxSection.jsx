        ...
        {!isOpen ? (
          <div className="flex flex-col items-center justify-center py-6">
            <button
              ...
              className="group relative flex flex-col items-center justify-center p-8 sm:p-10 rounded-3xl luxury-birthday-frame border border-[#F4D58D]/40 hover:border-[#E8A5B5]/60 transition-all duration-500 hover:scale-105 active:scale-95 shadow-2xl cursor-pointer"
            >
              {/* Glowing Box Icon */}
              <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-gradient-to-tr from-[#1A1326] to-[#342247] flex items-center justify-center text-4xl sm:text-5xl border border-[#F4D58D]/30 shadow-lg group-hover:rotate-6 transition-transform">
                🎁
              </div>

              <div className="mt-6 inline-flex items-center gap-2 px-7 py-3 rounded-full bg-gradient-to-r from-[#F4D58D] via-[#E8A5B5] to-[#F4D58D] text-[#070812] font-semibold text-sm shadow-lg">
                <span>Open Birthday Gift</span>
                <Sparkles className="w-4 h-4 text-[#070812]" />
              </div>
            </button>
          </div>
        ) : (
          <div className="luxury-birthday-frame p-8 sm:p-14 md:p-16 max-w-3xl mx-auto space-y-8 animate-fade-in shadow-2xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left pt-2">
              {giftPillars.map((item, idx) => (
                <div key={idx} className="birthday-note-tag p-5 space-y-1">
                  ...
                  <p className="text-xs text-slate-300 font-light pl-4">
                    {item.desc}
                  </p>
                </div>
              ))}
            ...
            <div className="p-6 rounded-2xl bg-[#120D1C]/80 border border-[#F4D58D]/25 text-center space-y-2">
