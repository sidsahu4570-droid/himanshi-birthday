...
export default function OpeningScreen({ onOpen, setIsMusicPlaying }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center bg-[#070812]/95 backdrop-blur-2xl transition-all duration-1000 px-4 sm:px-6 ${
        isOpening ? 'opacity-0 pointer-events-none scale-105' : 'opacity-100'
      }`}
    >
      {/* Warm Midnight Candlelight Glow */}
      ...
      <div className="max-w-md w-full text-center relative z-10 space-y-7 sm:space-y-8 animate-fade-in">
        ...
        {/* Soft Golden Heart Charm */}
        <div className="relative inline-flex items-center justify-center w-18 h-18 sm:w-20 sm:h-20 mb-1 rounded-full luxury-birthday-frame border border-[#F4D58D]/35 shadow-2xl shadow-[#F4D58D]/15 group">
          <Heart className="w-7 h-7 sm:w-8 sm:h-8 text-[#E8A5B5] fill-[#E8A5B5]/20 animate-pulse" />
        ...
        {/* Action Button - STRICTLY ONE LINE */}
        <div className="pt-2 sm:pt-4">
          <button
            onClick={handleOpen}
            disabled={isOpening}
            className="group relative inline-flex items-center justify-center gap-2 sm:gap-2.5 px-5 sm:px-7 py-3.5 sm:py-4 rounded-full bg-gradient-to-r from-[#F4D58D] via-[#E8A5B5] to-[#F4D58D] text-[#070812] font-semibold text-xs sm:text-sm md:text-base shadow-xl shadow-[#F4D58D]/20 hover:shadow-[#E8A5B5]/30 border border-white/40 hover:scale-[1.03] active:scale-[0.98] transition-all duration-300 cursor-pointer whitespace-nowrap max-w-full"
          >
            <span className="tracking-wide whitespace-nowrap">There's something I made for you</span>
            <Sparkles className="w-4 h-4 sm:w-4.5 sm:h-4.5 text-[#070812] shrink-0 group-hover:rotate-12 transition-transform" />
          </button>
        </div>
